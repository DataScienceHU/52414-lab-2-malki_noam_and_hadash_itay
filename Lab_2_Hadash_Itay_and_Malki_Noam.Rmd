...
Also we can se that the Switzerland Super League have especially bad players because,their Q.25 is low compared to the other leagues.
...
The logrithem scale of the wage density is a better visualization of than the <br> 
non logrithem scale because it lets the viewer the density of players who make a high salary next to the players who make a low one. <br> 
Unlike the non logrithem plot that show mostely the playres who make low salary. <br>
...
Also, becaue the log wage distribution has a skewness of -1.503398 that this <br> distribution has a "fat tail" on the negative side of the x axis.<br>

The log wage distributiom is described by a Normal distribution from two reasons.<br>
...
<br>**Answer**<br>
I will recommend to travel to Algeria that has 57 players and have a Overall average of 70.94737
...
```
<br>**Answer**<br>
We should expect to find players for future development based on this graph<b>
on the ages of 16/17, because those are the ages that have the most "space" for<br> future development between their Overall ability and thier Potential.  
<br>**e)**<br>
...
cor_pos_attri %>% ggcorrplot() + scale_x_discrete(limit = colnames(attri)) + labs(title = "correlation between chosen positions and Overall and playres skils") + theme(title = element_text(color="black",size=8,face="bold"),axis.ticks = element_line())
...
best_team <- as.data.frame(as.matrix(rbind(best_CAM,best_CB,best_CM,best_RB,best_RW,best_ST)))
best_team <- best_team %>% select(-V1)
best_team <- as.data.frame(rbind(rep(100,6),rep(45,6),best_team))
rownames(best_team) <- c("max","min","L.Messi" , "Sergio Ramos" , "T.Kroos" , "	Marcelo" , "Neymar" , "Cristiano Ronaldo" )
colnames(best_team)<- c("CAM","CB","CM","RB","RW","ST")
radarchart(best_team,seg = 5,plwd = 2,axistype = 2,plty = 1,palcex = 0.8,vlcex = 0.7,pcol = c("red",
"deeppink","dodgerblue4","gold","burlywood4","green4"),title = "DREAM TEAM")

legend(x=1.1, y=1, legend = rownames(best_team)[3:8], bty = "n", pch= 16 , text.col = "black",col =c("red",
"deeppink","dodgerblue4","gold","burlywood4","green4") , cex=0.8, pt.cex=1)
...
g <- g + geom_bar(stat="identity",color="black",width = 0.5) + theme(axis.text.x =element_blank(),axis.ticks.x = element_blank()) + labs(x="",y="diversity",title ="diversity of different nationalities in each club" ) + coord_cartesian(ylim = c(0,0.15))
g
```
