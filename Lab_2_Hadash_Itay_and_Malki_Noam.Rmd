...
=======
wage_avg <- function(potential){
  result <- fifa_players_info %>% filter(Overall==potential) %>% summarise(mean_wage = mean(Wage))
  return(as.integer(ceiling(result)))
}

wage_calculator <-function(potential,wage) {
  ifelse(potential <= 83, new_wage <- 10000, new_wage <- ceiling(1.5 * wage + 250 * exp(potential - 80)))
  return(new_wage)
}
>>>>>>> a4984b47db5c6c95152d910817e3dd1067860dbb

top_10_undervalued <- data.frame(head(undervalued_players, 10))
top_10_undervalued <- mutate(top_10_undervalued, wage_proposition = wage_calculator(Potential, Wage))

colnames(top_10_undervalued)
```
...
melt_attri %>% ggplot(aes(x=Var1,Var2,fill=value)) + geom_tile() + scale_fill_gradient(low="blue", high="red") + 
  theme_bw() +  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + labs(title = "correlation betweem diffrent abilitys")
...
cor_pos_attri %>% ggcorrplot() + scale_x_discrete(limit = colnames(attri)) + labs(title = "correlation between chosen positions and Overall and playres skils")
...
left_join(id_age,skill,by = "ID") %>% select(-ID) %>% group_by(Age) %>% summarise_all(list(mean)) %>% filter(Age<=35) %>% melt(id="Age") %>% ggplot(aes(x=Age,y=value,color=variable)) + geom_line(lwd=1.1) + coord_cartesian(ylim = c(30,70),xlim = c(16,35)) 
